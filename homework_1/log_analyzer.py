...
def calculate_statistics(file_path: str, log_parser: Callable, errors_limit: float = None) -> Dict:
    """
    ...
    if errors_limit is not None and total > 0:
        ...
        if cur_errors_limit:
            raise Exception(f'Errors limit = {errors_limit} was exceed, current errors limit={cur_errors_limit}')
...
def render_template(template_file_path: str, report_file_path: str, statistics: list[dict]) -> None:
    """
...
def main(config: dict) -> None:
    ...
    if not os.path.isdir(config['REPORT_DIR']):
        ...
        logging.info(f"Create report directory {config['REPORT_DIR']} because it doesn't exist.")
